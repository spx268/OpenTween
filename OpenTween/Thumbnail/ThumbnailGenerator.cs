...
#region " IDisposable Support "
        ...
        public ThumbnailGenerator(TweenMain Owner)
        {
            ...
            ThumbnailServices = new[] {
                ...
                new ThumbnailService("gyazo", gyazo_GetUrl, gyazo_CreateImage),
...
#region "Twitter公式"
        ...
        private bool gyazo_GetUrl(GetUrlArgs args)
        {
            var mc = Regex.Match(string.IsNullOrEmpty(args.extended) ? args.url : args.extended,
                                          @"^http://gyazo\.com/(\w+)(?:\.png)?$", RegexOptions.IgnoreCase);
            if (mc.Success)
            {
                args.imglist.Add(new KeyValuePair<string, string>(args.url, mc.Result("http://gyazo.com/${1}.png")));
                return true;
            }
            else
            {
        ...
        private bool gyazo_CreateImage(CreateImageArgs args)
        {
            var img = (new HttpVarious()).GetImage(args.url.Value, args.url.Key, 10000, out args.errmsg);
            if (img == null)
            {
